...
!  and send this data to the other procs.
  if (myid==0) then
    ...
    write(6,*) "topband", topband
    write(6,*) "midband", midband
...
&            buffCal_z(iband)%b(10*Ngal(2,iband)+19))
    ...
    ! write(*,*) 'iband = ', iband
    ! write(*,*) '  size(buffR_x(iband)%b)   = ', size(buffR_x (iband)%b)
    ! write(*,*) '  size(buffC_z(iband)%b)   = ', size(buffC_z (iband)%b)
    ! write(*,*) '  size(buffRal_x(iband)%b) = ', size(buffRal_x(iband)%b)
    ! write(*,*) '  size(buffCal_z(iband)%b) = ', size(buffCal_z(iband)%b)

      ! if( iband==3) then
      !   write(6,*) "buffCal_z before"
      !   do i =1,114
      !     write(*,*) buffCal_z(iband)%b(i)
      !   end do
      ! end if

      ! if( iband==3) then
      !   write(6,*) "buffral_z before"
      !   do i =1,size(buffRal_x  (iband)%b)
      !     write(*,*) buffRal_x  (iband)%b(i)
      !   end do
      ! end if
...
!     dnz = Ngal(2,1)/ntilez
  ...
  do iband = sband,eband
    ...
    allocate( wv_f        (iband)%f   (  jlim(1,vgrid,iband)  :jlim(2,vgrid,iband)  ,columns_num(iband,myid)))
...
!  yu(nn+dny+1)=-yu(-dny) !E!2
 ...
 !   !'Wall' grid (just smooth channel) ------------------------------------------------------------------------
  ...
  ! Analytical expression for dtheta/dy in the lineal regions, and the polinomial one.
    ...
    ! do j=0,nn
    !  write(6,*) "dthdyv=", dthdyv(j), "dthdyu",dthdyu(j+1)
    ! end do


    ! do j=0,nn
    !  write(6,*) "j=", j, "yv=", yv(j), "yu",yu(j+1), "dthdyv=", dthdyv(j), "dthdyu",dthdyu(j+1)
    ! end do
...
! Second-order finite difference coefficient for 1. u(j-1), 2. u(j), 3. u(j+1)
  ...
  ! write(6,*) "N before spltting into bands?:"
  ! do i = 1,4
  !   write(6,*) N(i,0:4)
  ! end do
  ...
  do iband = 1,botband
    ...
    ! write(6,*) "iband", iband, "yu(0)=", yu(0)
    ! write(6,*) "h_ny=", h_ny(iband)
  ...
  ! write(6,*) "N after spltting into bands?:"
  ! do i = 1,4
  !   write(6,*) N(i,0:4)
  ! end do
  ...
  ! write(6,*) "N after adding yv grid?:"
  ! do i = 1,4
  !   write(6,*) N(i,0:4)
  ! end do
  ...
  ! write(6,*) "should be the same as above:"
  ! do i = 1,4
  !   write(6,*) Ngal(i,0:4)
  ! end do
...
!    k2F_z(N(2,1)/2+1) = 0d0 !These are now included
  ...
  allocate(iLkup(-N(1,1)/2:N(1,1)/2), kLkup(-N(2,1)/2:N(2,1)/2), iNeg(-N(1,1)/2:N(1,1)/2))
  ...
  kLkup(N(2,1)/2) = -N(2,1)/2 + 1 + Ngal(2,1)
  do k = -N(2,1)/2,-1
    kLkup(k) = k + 1 + Ngal(2,1)
    !write(6,*) "kLkup", kLkup(k)
  end do
...
subroutine proc_lims_columns(myid)
  ...
  ! write(*,*) 'columns_num_short_proc     = ', columns_num_short_proc
  ! write(*,*) 'columns_num_short_proc_rem = ', columns_num_short_proc_rem
  ! write(*,*) 'columns_num_long_proc      = ', columns_num_long_proc
  ! write(*,*) 'columns_num_long_proc_rem  = ', columns_num_long_proc_rem
  ...
  do iproc = 0,np-1
    ...
    ! write(6,*) "columns_i =", columns_i(column, midband, iproc),  &
    !           " columns_k =", columns_k(column, midband, iproc), &
    !           " columns_long_list_i =", columns_long_list_i(column + accum), &
    !           " columns_long_list_k =", columns_long_list_k(column + accum)
  ...
  ! write(*,*) 'jlim = ',  &
  !           jlim(1,ugrid,1), jlim(2,ugrid,1), &
  !           jlim(1,ugrid,2), jlim(2,ugrid,2), &
  !           jlim(1,ugrid,3), jlim(2,ugrid,3)
  ...
  ! write(*,*) 'vgrid band2 jlim = ', jlim(1,vgrid,2), jlim(2,vgrid,2)
  ...
  ! write(*,*) 'pgrid band2 jlim = ', jlim(1,pgrid,2), jlim(2,pgrid,2)
  ...
  !       do column = 1,columns_num(iblock,iproc)
  !         if (columns_k(column,iblock,iproc) > N(2,1)/2) then
  !           write(6,*)'col_k=',columns_k(column,iblock,iproc), 'dk=', dk(column,iblock,iproc,iband),"iproc", iproc
  !         end if
  !       end do
  !     end do
  !   end do
  ...
  do iproc = 0,np-1
    ...
    do iblock = sband,eband
      do iband = 2,2
        do column = 1,columns_num(iblock,iproc)
          if (columns_k(column,iblock,iproc) > N(2,1)/2) then
            ! write(6,*)'col_k=',columns_k(column,iblock,iproc), 'dk_phys=', dk_phys(column,iblock,iproc,iband),"iproc", iproc
          end if
        end do
      end do
    end do
...
subroutine proc_lims_planes(myid)
  ...
  !   write(*,*) 'iproc =', iproc, ' limPL_FFT(:,1:2,iproc) =', limPL_FFT(:,1,iproc), limPL_FFT(:,2,iproc)
