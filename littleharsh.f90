...
nstatsl = nwrite
  ...
  do iband = sband,eband
    ...
    allocate( a(ugrid,iband)%fr(3,jlim(1,ugrid,iband):jlim(2,ugrid,iband)) )
    allocate( a(vgrid,iband)%fr(3,jlim(1,vgrid,iband):jlim(2,vgrid,iband)) )
  end do
...
&                                    -p (iband)%f(j,  column))
    ...
    !   write(6,*) 'du1, j=1..10:'
    !   do j = 1, 10
    !     write(6,*) real(du2(iband)%f(j,column)), aimag(du2(iband)%f(j,column)), iband 
    !   end do
    
      ! write(6,*) 'du1, j=1..10:'
      ! do j = columns_num(iband,myid) -10, columns_num(iband,myid)
      !   write(6,*) real(du2(iband)%f(j,column)), aimag(du2(iband)%f(j,column)), iband 
      ! end do 
    ! end if 
    ...
    ! if (column == 1 .and. myid == 0) then
    !   do j = 1, 10
    !     write(6,*) 'nu2, p and p, 1-10 '
    !     write(6,*) Nu2(iband)%f(j  ,column), p(iband)%f(j+1,column),  -p(iband)%f(j,  column)
    !   end do

      ! do j = columns_num(iband,myid) -10, columns_num(iband,myid)
      !   write(6,*) 'nu2, p and p, -10 to top '
      !   write(6,*) Nu2(iband)%f(j  ,column), p(iband)%f(j+1,column),  -p(iband)%f(j,  column)
      ! end do
    ! end if
...
&                              + C3* p (iband)%f(j,column)
    ...
    !   write(6,*) 'du3, j=1..10:'
    !   do j = 1, 10
    !     write(6,*) real(du3(iband)%f(j,column)), aimag(du3(iband)%f(j,column))
    ...
    !   do j = 1, 10
    !     write(6,*) 'nu2, p and p, 1-10 '
    !     write(6,*) Nu3(iband)%f(j  ,column), p(iband)%f(j,column)
    !   end do
    ! end if 
...
subroutine solveU_W(u,du,w,dw,a,myid) !pass (u,du,w,dw)
   ...
   type(cfield) dw(sband:eband)
   type(rfield) a(2,sband:eband)
   ...
   do iband = sband,eband
      do column = 1,columns_num(iband,myid)
         ...
         dw(iband)%f(jlim(1,ugrid,iband),column) = 0d0

         do j = jlim(1,ugrid,iband)+1,jlim(2,ugrid,iband)-1
            du(iband)%f(j,column) = u(iband)%f(j,column)+dt*(du(iband)%f(j,column)) !For solving for u
            dw(iband)%f(j,column) = w(iband)%f(j,column)+dt*(dw(iband)%f(j,column)) !For solving for w
         end do

         du(iband)%f(jlim(2,ugrid,iband),column) = 0d0
         dw(iband)%f(jlim(2,ugrid,iband),column) = 0d0
      end do
   end do
   ...
   do iband = sband,eband
      call LUsolU_W(u,du,w,dw,a(ugrid,iband)%fr(1:3,jlim(1,ugrid,iband):jlim(2,ugrid,iband)),ugrid,iband,myid)
   end do
end subroutine
...
subroutine solveV(u,du,a,myid)
   ...
   do iband = sband,eband
      call LUsolV(u,du,a(vgrid,iband)%fr(1:3,jlim(1,vgrid,iband):jlim(2,vgrid,iband)),vgrid,iband,myid)
   end do
end subroutine
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do iband = 1,2
    do column = 1,columns_num(iband,myid)
      u1(iband)%f(jlim(1,ugrid,iband),column) = gridweighting_bc_u1*u1(iband)%f(jlim(1,ugrid,iband)+1,column)
    enddo
  enddo
  
  do iband = 2,3
    do column = 1,columns_num(iband,myid)
      u1(iband)%f(jlim(2,ugrid,iband),column) = gridweighting_bc_u1*u1(iband)%f(jlim(2,ugrid,iband)-1,column)
    enddo
  enddo
...
!       u2(iband)%f(jlim(2,vgrid,iband),column) = 0d0
 ...
 u2(iband)%f(jlim(2,vgrid,iband),column) = 0d0
  ...
  do iband = 1,2
    do column = 1,columns_num(iband,myid)
      u3(iband)%f(jlim(1,ugrid,iband),column) = gridweighting_bc_u3*u3(iband)%f(jlim(1,ugrid,iband)+1,column)
    enddo
  enddo
  
  do iband = 2,3
    do column = 1,columns_num(iband,myid)
      u3(iband)%f(jlim(2,ugrid,iband),column) = gridweighting_bc_u3*u3(iband)%f(jlim(2,ugrid,iband)-1,column)
    enddo
  enddo
